...
def main() -> None:
    ...
    # 9. 打印使用的关键配置（用于确认）
    print("\n" + "="*60)
    print("📋 GhostNetV2 Backbone 训练配置摘要")
    print("="*60)
    print("🔧 模型组件:")
    print("  - Backbone: GhostNetV2（GhostConv + C3Ghost + DFC Attention）")
    print("  - Neck: DDPG + BiFPN_DWC（保持不变）")
    print("  - Head: 标准检测头（保持不变）")
    print("  - Loss: CSL_ProbIoU（保持不变）")
    print("="*60)
    print("📐 骨干网络结构:")
    print("  Conv → GhostConv → C3Ghost → DFC Attention →")
    print("  GhostConv → C3Ghost → DFC Attention →")
    print("  GhostConv → C3Ghost → DFC Attention →")
    print("  GhostConv → SPPF → C2PSA")
    print("="*60)
