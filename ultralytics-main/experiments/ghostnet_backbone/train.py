...
def main() -> None:
    ...
    if "model" in exp_cfg and exp_cfg["model"]:
        ...
        print(f"✅ 使用GhostNet模型配置: {model_path}")
    else:
        # 默认使用本地预训练权重
    ...
    # 9. 打印使用的关键配置（用于确认）
    print("\n" + "="*60)
    print("📋 GhostNet Backbone 训练配置摘要")
    print("="*60)
    print("🔧 模型组件:")
    print("  - GhostNet Backbone: GhostConv + C3Ghost (轻量级)")
    print("  - Standard FPN Head: 标准特征金字塔网络")
    print("  - RAL Loss: 旋转感知损失函数（自动启用）")
    print("="*60)
