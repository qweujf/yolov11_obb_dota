...
def main() -> None:
    ...
    # 4. 处理模型路径 - 使用 YOLOv8n-OBB
    model_path: str = ""
    if "model" in exp_cfg and exp_cfg["model"]:
        # 实验配置中指定了模型
        model_path = str(Path(exp_cfg["model"]).expanduser().resolve())
    else:
        # 使用 YOLOv8n-OBB 预训练权重
        # 尝试本地权重，如果不存在则使用模型名称（会自动下载）
        local_weights = repo_root / "yolov8n-obb.pt"
        if local_weights.exists():
            ...
            print(f"✅ 使用 YOLOv8n-OBB 模型: {model_path}（将自动下载预训练权重）")
            cfg["pretrained"] = True
